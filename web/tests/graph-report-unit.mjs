...
assert(thresholdSettings.includes('1.5 m · level'));
assert(thresholdSettings.includes('1.6 m · level'));
...
sandbox.document.getElementById=id=>({value:id==='gapInput'?'900':id==='rainFactor'?'1':(id==='analysisStart'||id==='analysisEnd')?'':'#ff0000'});
await run(`(async()=>{
 state.files.set('test',{id:'test',displayName:'sample.csv',virtualPath:'/sample.csv',parsed:{start:'2026-01-01T00:00:00',end:'2026-01-01T01:00:00',metadata:{quantity:'depth',canonical_unit:'m'}}});
...
const neutralPeriod=run(`reportAnalysisPeriod()`);
assert.deepEqual(Array.from(neutralPeriod),['2026-01-01T00:00:00','2026-01-01T01:00:00']);
assert(!neutralPeriod.some(x=>String(x).endsWith('Z')));
...
const clippedLogFit=run(`regressionLinePoints({regression_slope:2,regression_intercept:-3},[{obs:1,sim:1},{obs:4,sim:5}],true)`);
assert(clippedLogFit,'A raw-scale fit crossing zero must retain its positive segment on a log view.');
assert(clippedLogFit.x[0]>1.5&&clippedLogFit.x[1]===4);
